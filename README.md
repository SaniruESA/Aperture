...
The large size of the Aperture application is due in part to its self-contained nature; eliminating the need for installing external requirements requires us to package all dependencies within this single executable. Additionally, many AI models part of the Aperture Library (e.g. Whisper for audio transcription) are installed locally to ensure that users of an Aperture-edited repository have full data safety with internal use of AI models. In the future, we will continue to optimize the software to reduce its working size.
