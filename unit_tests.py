...
def test_stats():
    ...
    for test in TESTS:
        ...
        print(f"{("\x1b[32m" if test["pass"] else "\x1b[31m")}{test["name"]}: {("Passed" if test["pass"] else "Fail")}\x1b[0m")
    
    print("------------------")
    print(f"{"\x1b[33m" if TOTAL_PASS < TOTAL_TESTS else "\x1b[32m"}({TOTAL_PASS}/{TOTAL_TESTS}) Passed")
...
def test_4_popup():
    ...
    for element in elements:
        ...
        if element["text"] == "Hello, World!" and element["text-color"] == [0,255,0] and element["background-color"] == [255,0,0] and element["position"] is None:
