...
class TabNavOrder:
    ...
    def handleTabPress(self, shift: bool = False):
        ...
        if ui.is_button_highlighted:
            self.tabbedElementIndex = (
                self.tabbedElementIndex + (-1 if shift else 1)
            ) % len(self.order["button"])
        else:
        ...
        # Narrate the text
        server_ticker.SERVER.tts_queue.queue_play(
            f".\\temp\\output_{self.tabbedElement['ariaText']}.mp3",
            -1,
            self.tabbedElement["ariaText"],
            True,
            True,
        )
        ...
        info(
            f"Switched tab focus to element named {self.tabbedElement["ariaText"]}",
            __name__,
        )
        ...
        for UIElement in flattenedUIElements:
            ...
            if rect_point_collision(UIElement["rect"], mousePos):
                ...
                if self.hoveredElement["ariaText"] != UIElement["ariaText"]:
                    server_ticker.SERVER.tts_queue.queue_play(
                        f".\\temp\\output_{UIElement['ariaText']}.mp3",
                        99,
                        UIElement["ariaText"],
                        True,
                        True,
